	...
	if (!(RootPtr == NULL)){
		if (RootPtr -> Key == K){
			*Content = RootPtr -> BSTNodeCont;
			return TRUE;
			//If the required item was found, set its' content to *Content
		}

		else if (K > RootPtr -> Key)
			return BSTSearch(RootPtr -> RPtr, K, Content);
			//If the required items key is bigger, we choose to go right

		else 
			return BSTSearch(RootPtr -> LPtr, K, Content);
			//If the required items key is smaller, we choose to go left
	}	
	else		   
		return FALSE;	
		//Return false when there are no more items
...
**/
	...
	if (*RootPtr == NULL){
		*RootPtr = malloc(sizeof(struct tBSTNode));
		...
		return;
		//If we the allocation failed, return out of function
	}

	if(K > (*RootPtr) -> Key)
		BSTInsert(&(*RootPtr) -> RPtr, K, Content);
		//If K is greater than the key, we move to the right
	else if (K < (*RootPtr) -> Key)
		BSTInsert(&(*RootPtr) -> LPtr, K, Content);
		//If K is less than they key, we move to the left
	else
		(*RootPtr) -> BSTNodeCont = Content;	
...
**/
	...
	if ( !(RootPtr == NULL) && !((*RootPtr) == NULL)){
		if (!((*RootPtr) -> RPtr == NULL))
			...
			//If we are not in the rightmost node, we continue
		else{
...
**/
	...
	if (RootPtr && (*RootPtr)){
		if ((*RootPtr) -> Key > K)
			...
			//Key is smaller than active key, move to the left
		else if ((*RootPtr) -> Key < K)
			BSTDelete(&(*RootPtr) -> RPtr, K);
			//Key is bigger than active key, move to the right
