...
**/
	...
	if (!(*RootPtr == NULL)){
		...
		tBTNodePtr insertNode = NULL;

		while (insertNode == NULL){
		//Iterating while the node is not inserted
			if (helper -> Cont < Content){
				//The value is bigger, we use the right subtree
				if (!(helper -> RPtr == NULL))
					helper = helper -> RPtr;
					//If we can, we move to the right subtree
				else {
					insertNode = malloc(sizeof(struct tBTNode));
					//There is no right subtree so we add a new node
					if (!(insertNode == NULL)){
						insertNode -> LPtr = NULL;
						insertNode -> RPtr = NULL;
						insertNode -> Cont = Content;
						helper -> RPtr = insertNode;
					}
					else return;
					//If there was a problem with the allocation
				}
			}

			else if (helper -> Cont > Content){
				//The value is smaller, we use the left subtree
				if (!(helper -> LPtr == NULL))
					helper = helper -> LPtr;
					//If we can, we move to the left subtree
				else {
					insertNode = malloc(sizeof(struct tBTNode));
					//There is no left subtree so we add a new node
					if (!(insertNode == NULL)){
						insertNode -> LPtr = NULL;
						insertNode -> RPtr = NULL;
						insertNode -> Cont = Content;
						helper -> LPtr = insertNode;
					}
					else return;
					//If there was a problem with the allocation
				}
			...
			//The node is existing
		}
	}

	else {
		//The tree is empty, adding first node
...
**/
	...
	while (!(ptr == NULL)){
		...
		//Put node into stack
		SPushB(StackB, true);
		//Set the value so that we know it was visited
		ptr = ptr -> LPtr;
	...
	if (!(RootPtr == NULL)){
		...
		if (!(StackP == NULL) && !(StackB == NULL)){
			...
			//Iterating, while the stack is not empty
				...
				bool tempB = STopPopB(StackB);
				if(tempB){
					//This node has been visited, push it back to stack
					SPushB(StackB, false);
					...
					BTWorkOut(RootPtr);
					//This node has not been visited yet, so we work with it
			}
...
**/
	if (!(*RootPtr == NULL)){
		...
		if (!(Stack == NULL)){
			...
			//Pushing stack to the top of the tree

			while (!(SEmptyP(Stack))){
			//Iterating, while the stack is not empty
				helper = STopPopP(Stack);
				//Going from the top node
				if (!(helper -> RPtr == NULL))
					SPushP(Stack, helper -> RPtr);
					//Pushing the right subnode to stack
				if (!(helper -> LPtr == NULL))
					SPushP(Stack, helper -> LPtr);
					//Pushing the left subnode to stack
		...
		//Return if there was a problem with allocation
