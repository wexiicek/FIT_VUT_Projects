...
*/
	...
	if(q != NULL){
		for(int i = 0; i < QUEUE_SIZE; i++)
			q -> arr[i] = '*';
			//If Q is not NULL, we set every element to '*'

		q -> f_index = 0;
		//Then we set the first position index to the value of 0 [first element]
		q -> b_index = 0;
		//Same for first available index [it is the first element]
		return;
	}
	queueError(QERR_INIT);
}
...
*/
	...
	//If the index of the first element is the same as the
	//index of the first available element, the queue is empty.
}
...
*/
	...
	//If the next element in the queue is the first, we know
...
*/
	...
	*c = q -> arr[q -> f_index];
	//If the queue is NOT empty, set the pointer c to the value of the first element
}
...
*/
	...
	//If the queue is NOT empty, we put the second element into the index of the first one
}
...
*/
	...
	queueFront(q, c);
	queueRemove(q);
	//If the queue is not empty, we use 
	//queueFront to return the element using the c pointer
	//queueRemove to remove the element from the queue
}
...
*/
	...
	q -> arr[q -> b_index] = c;
	//If the queue is NOT full, we set the value of c
	//to the first available position in the queue (b_index)
	q -> b_index = nextIndex(q->b_index);
	//Then we move the first available index past the new element
	if(q -> b_index == QUEUE_SIZE)
		q -> b_index = 0;
}
